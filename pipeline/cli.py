...
from models import run_cnn_model, get_cnn_model
...
def generate_heatmap_realtime():
    while True:
        if not is_dir_empty(openpose_processing_folder):
            rd.set('OPENPOSE_IN_PROGRESS', '1')
            generate_heatmap_with_openpose(openpose_processing_folder)
...
if __name__== "__main__":
    ...
    elif arg == 'generate_keypoints_realtime':
        generate_keypoints_realtime()
    elif arg == 'predict_realtime':
        predict_realtime()
